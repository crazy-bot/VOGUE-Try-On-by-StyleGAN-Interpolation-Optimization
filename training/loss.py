...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_encoder, D, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        super().__init__()
        self.device = device
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, gen_pose, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_pose, sync=(sync and not do_Gpl)) # May get synced by Gpl.
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                gen_img, gen_ws = self.run_G(gen_z[:batch_size], gen_pose[:batch_size], sync=sync)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_pose, sync=False)
